...
impl JjClient {
    ...
    pub fn begin(&self) -> Result<JjSession> {
...
impl JjSession {
    ...
    pub fn read_entry_at_rev(
        ...
        let commit = self.resolve_summary_to_commit(rev)?;
        let repo_path = self.parse_repo_path(path)?;
        self.read_entry_from_tree(self.repo(), &commit.tree(), repo_path.as_ref(), path)
            .with_context(|| {
                format!(
                    "failed to read `{}` from revision `{}`",
                    path.display(),
    ...
    pub fn set_bookmark(&mut self, name: &str, rev: &RevisionSummary) -> Result<()> {
        let commit = self.resolve_summary_to_commit(rev)?;
        self.tx
            .repo_mut()
            .set_local_bookmark_target(name.as_ref(), RefTarget::normal(commit.id().clone()));
    ...
    pub fn clear_bookmark(&mut self, name: &str) -> Result<()> {
        self.tx
            .repo_mut()
            .set_local_bookmark_target(name.as_ref(), RefTarget::absent());
        self.dirty = true;
    ...
    pub fn complete_sync(&mut self, exported: &RevisionSummary) -> Result<()> {
    ...
    pub fn checkout_revision(&mut self, rev: &RevisionSummary) -> Result<()> {
        let commit = self.resolve_summary_to_commit(rev)?;
        self.tx
            .repo_mut()
